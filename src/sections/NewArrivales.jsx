...
const NewArrivals = () => {
    ...
    return (
        <section className={`${darkMode ? 'dark bg-[#19191a]' : 'light bg-slate-200'} w-full lg:px-20 px-10 lg:py-20 py-10 flex flex-col justify-center items-center gap-6`}>
            <h1 data-aos="zoom-in" className=' uppercase text-2xl text-slate-500 dark:text-white text-center'>Top Products</h1>
            <h1 data-aos="zoom-in" className='text-5xl font-semibold text-black dark:text-white text-center'>New Arrivals</h1>

            <div data-aos="slide-up" className='w-full flex lg:flex-row flex-col justify-center items-center flex-wrap gap-8 mt-3'>
                {
                    newarrivals.map((item, index) => (
                        <div key={index} id='product-box' className='flex flex-col justify-center items-start gap-2 bg-white dark:bg-black rounded-xl'>
            ...
            <button data-aos="zoom-in" className=' text-black text-md font-semibold px-8 py-4 rounded-md transform hover:scale-105 transition-transform duration-300 mt-10 bg-transparent border-black border-2 dark:border-white dark:text-white'>View All Products</button>
        </section>
    )
}
